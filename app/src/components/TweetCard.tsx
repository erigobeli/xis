...
import { deletePost, fetchComments, type FeedPost, type PostEngagement } from '@/lib/posts'
import { splitPostContent } from '@/lib/post-text'
...
function renderPostContent(content: string) {
  return splitPostContent(content).map((segment, index) => {
    if (segment.type === 'hashtag') {
      return (
        <span
          key={`${segment.value}-${index}`}
          className="rounded-md bg-accent/10 px-1.5 py-0.5 font-medium text-accent hover:bg-accent/15"
        >
          {segment.value}
        </span>
      )
...
export default function TweetCard({ post, engagement, onDeleted, onUpdated, compact = false, depth = 0 }: TweetCardProps) {
  ...
  const handleDelete = async () => {
    ...
    const result = await deletePost(post.id, post.image_url)
    if (result.imageCleanupFailed) {
      alert('A publicacao foi excluida, mas nao foi possivel remover a imagem do storage.')
    }
    onDeleted?.()
  }
    ...
    >
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {post.image_url && (
            <div className="mt-3 overflow-hidden rounded-2xl border border-border bg-bg-secondary">
              <img
                src={post.image_url}
                alt=""
                className="h-auto w-full max-h-[540px] object-cover"
                loading="lazy"
              />
            </div>
          )}
